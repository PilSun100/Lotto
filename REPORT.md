...
일반 사용자 기능은 Django 템플릿 기반 웹 화면으로 구현했습니다. 사용자는 `/accounts/signup/`에서 가입하고 `/tickets/buy/`에서 복권을 구매합니다. 수동 구매는 1부터 45 사이의 중복 없는 숫자 6개를 입력해야 하며, 자동 구매는 서버가 `random.sample`을 사용해 중복 없는 번호 6개를 생성합니다. 추첨 이후에는 `/tickets/check/` 화면에서 최근 당첨 번호와 본인 복권의 당첨 결과를 확인할 수 있습니다.

관리자 기능은 Django admin 중심으로 구현했습니다. 관리자는 `Round`를 생성하여 판매 기간을 열 수 있고, 회차 목록에서 admin action `선택한 회차 추첨 실행`을 실행해 당첨 번호와 보너스 번호를 랜덤 생성합니다. `DrawResult`는 직접 입력하지 못하도록 읽기 전용으로 제한하여, 모든 당첨 번호가 회차별 추첨 기능을 통해서만 생성되도록 했습니다. 추첨이 완료되면 `Ticket` 목록과 사용자 `내 복권` 화면에서 당첨 등수가 표시됩니다.
...
- 추첨 전후 당첨 결과 표시 변화 및 당첨 확인 화면 검증
- Django admin action으로 추첨 결과가 생성되는지 검증
- 추첨 결과 admin에서 직접 추가가 차단되는지 검증
